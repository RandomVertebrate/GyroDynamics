...
TankPitch = (time^3/(0.001+time^3))*(10*pi/180)*sin(2*pi*30*time);
TankYaw = (time^3/(0.001+time^3))*(10*pi/180)*cos(2*pi*45*time);
...
xlabel('Time (sec)');
ylabel('Angle (deg)');
legend('Tank Roll', 'Tank Pitch', 'Tank Yaw', 'Location', 'southoutside', ...
...
xlabel('Time (sec)');
ylabel('Angle (deg)');
legend('L.o.S. Pitch', 'L.o.S. Yaw', 'Location', 'southoutside', ...
    'Orientation', 'horizontal');
